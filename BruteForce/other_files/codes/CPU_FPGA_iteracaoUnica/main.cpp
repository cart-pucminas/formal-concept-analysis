...
void CarregaContexto(){
  ...
  retKernel = (bool*)malloc(sizeof(bool));
...
void StartKernel(){
  ...
  retornoBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 
                  sizeof(bool), retKernel, &status); //buffer de 1 (apenas para retorno)
  ...
  status |= clSetKernelArg(kernel, 5, sizeof(int), &indice);
  
  status |= clFinish(queue);
	checkError(status, "Failed to set arguments");
}

bool ExecutarKernel(int ind){
  ...
  status |= clEnqueueReadBuffer(queue, retornoBuffer, CL_TRUE, 0, sizeof(bool), retKernel, 0, NULL, NULL);
  checkError(status, "Falha ao ler os Buffers");
  
  printf("\nteste: %d, id: %d", retKernel[0], ind);
